#pragma GCC optimize("O3")

#include<bits/stdc++.h>
#define sp " "
#define endl "\n"
#define MAX 10e9
#define MAXN 100001
#define mod 1000000007
#define inf_int 2e9
#define inf_ll 2e18
#define e 1e-9
#define vi vector<int>
#define vl vector<ll>
#define vc vector<char>
#define vs vector<string>
#define vpl vector<pair<ll,ll>>
#define vpc vector<pair<char,char>>
#define adj_list vector<vl>
#define umap unordered_map<ll, ll>
#define pll pair<ll,pair<ll, ll>>
#define clr(x) memset(x,0,sizeof(x))
#define cy cout << "YES" << endl
#define cn cout << "NO" << endl
#define google cout << "Case #" << ++it << ": " << ans << endl;
#define fastio ios_base::sync_with_stdio(false);cin.tie(NULL);cout.tie(NULL);
using namespace std;
typedef long long int ll;

ll it = 0, ans = 0;

ll spf[MAXN];
void sieve()
{
    spf[1] = 1;
    for (auto i=2; i<MAXN; i++)
  		spf[i] = i;
  	for (auto i=4; i<MAXN; i+=2)
        spf[i] = 2;
    for (auto i=3; i*i<MAXN; i++)
    {
        if (spf[i] == i)
        {
            for (auto j=i*i; j<MAXN; j+=i)
                if (spf[j]==j)
                    spf[j] = i;
        }
    }
	return;
}

vl getFactorization(ll x)
{
    vl ret;
    while (x != 1)
    {
        ret.push_back(spf[x]);
        x = x / spf[x];
    }
    return ret;
}


void check()
{
	ll n,x,y, maxx = 0;
	cin >> n >> x >> y;
	ll diff = y - x;
	vl div, res ;
	maxx = y;
	for(int i=1;i<=sqrt(diff);i++) {
		if(diff%i == 0) {
			if(diff/i == 0) div.push_back(i);
			else div.push_back(i), div.push_back(diff/i);
		}
	}
	for(auto i:div) {
		ll cnt = 0;
		vl ans;
		while(cnt < n and y-cnt*i >= 1) {
			ans.push_back(y-(cnt++)*i);
		}		
		if(ans.size() == n) {
			if(find(ans.begin(),ans.end(),x) != ans.end()) {				
				if(maxx >= *max_element(ans.begin(),ans.end()) or res.size() == 0) {
					res = ans;
					maxx = *max_element(ans.begin(),ans.end());
				}
			}
			else {
				continue;
			}
		}
		else {
			ll flag = 0;
			while(cnt < n) {
				if((y-cnt*i) < 1) {
					flag = 1;
					break;
				}
				ans.push_back(y-(cnt++)*i);
			}
			if(flag = 1) {
				reverse(ans.begin(),ans.end());
				cnt = 1;
				while(cnt < n and ans.size()!=n) ans.push_back(y+(cnt++)*i);			
			}
			if(ans.size() == n) {
				if(find(ans.begin(),ans.end(),x) != ans.end()) {
					if(maxx >= *max_element(ans.begin(),ans.end()) or res.size() == 0) {
						res = ans;
						maxx = *max_element(ans.begin(),ans.end());
					}
				}
				else {
					continue;
				}
			}
		}
	}
	sort(res.begin(),res.end());
	for(auto i:res) cout << i << sp;
	cout << endl;
    return ;
}

int32_t main()
{
    fastio
    ll t = 1;
    cin >> t;
    while(t--)
        check();
    return 0;
}
